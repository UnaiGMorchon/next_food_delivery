...
function RestaurantCard({ data }) {
 ...
 // Verificar si 'data' y las propiedades anidadas existen antes de acceder a ellas
 if (data && data.attributes && data.attributes.image && data.attributes.image.data && data.attributes.image.data[0] && data.attributes.image.data[0].attributes && data.attributes.image.data[0].attributes.url) {
  // Acceder a la propiedad 'url' de manera segura
  const imageUrl = data.attributes.image.data[0].attributes.url;

  // Renderizar el componente con la imagen
  ...
  // Manejar el caso en el que los datos no están disponibles o no tienen la estructura esperada
  // Puedes mostrar un mensaje de error o realizar alguna otra acción
  return <p>Los datos de la imagen no están disponibles.</p>;
}
